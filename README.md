# ELECTRONIC GREETING CARD WITH LED USING COIN CELL

## DESCRIPTION
An interactive greeting card project that utilizes a simple electronic circuit powered by a coin cell battery to light up an LED when gets pressed.

## MATERIALS REQUIRED
* LED
* COIN CELL BATTERY (3V)
* ALUMINIUM FOIL
* TAPE, GLUE
* COLOUR A4 PAPER

## PROCEDURE
1. Fix the LED at the center of the chart paper.
2. Connect the LED to the coin cell battery with correct polarity.
3. Stick the aluminium foil on the path where current flows.
4. Check whether the circuit works safely.

## WORKING
The battery supplies the DC power to the circuit. When the circuit gets closed the current flows and the LED glows.

## NOTES
* Connect the LED with correct polarity.
* Use a resistor to limit current.
* Avoid short-circuiting the battery.
* Ensure all connections are tight and secure.




**SIMULATION  MODEL**

<img width="577" height="288" alt="LED Simulation" src="https://github.com/user-attachments/assets/12d93c6f-9cdd-431e-9039-f23d9f65a863" />


**GREETING CARD**  

![Greeting Card](Greeting%20card%20project.jpg)
